        ...
        if ($this->ManageStock()) {
            ...
            if (!$this->getIsInStock()) {
                ...
                if (mage::getStoreConfig('advancedstock/general/restore_isinstock')) {
                    //set product out of stock if min qty greater than
                    if ($this->getMinQty() < $this->getAvailableQtyForSale())
    ...
    protected function _afterSave() {
        ...
        if ($this->getqty() != $this->getOrigData('qty')) {
            ...
            if ($qtyFromStockMovement != $this->getqty()) {
                ...
                mage::getModel('AdvancedStock/StockMovement')->createStockMovement($productId, $sourceWarehouseId, $targetWarehouseId, $diff, mage::helper('AdvancedStock')->__('Adjustment'), $additionalData);
            }
        }
        ...
        if ($qtyAfterSave > $qtyBeforeSave) {
            ...
            if ($reservedQty < $orderedQty) {
                ...
                mage::helper('BackgroundTask')->AddTask('Reserve product #' . $productId . ' for pending orders', 'AdvancedStock/Product_Reservation', 'reserveProductForPendingOrders', $productId
                );
            } else {
                //todo: error, reserved qty CAN NOT BE LOWER THAN ordered qty
            }
        }

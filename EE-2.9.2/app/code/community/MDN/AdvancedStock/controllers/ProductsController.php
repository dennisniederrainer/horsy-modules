...
class MDN_AdvancedStock_ProductsController extends Mage_Adminhtml_Controller_Action {
    ...
    public function EditAction() {
        ...
        if (!$product->getId())
        {
            ...
            $this->_redirect('AdvancedStock/Products/Grid');
        }
        else
        {
            $this->loadLayout();
    ...
    public function SaveAction() {
        ...
        try {
            ...
            foreach ($stocks as $stock) {
                ...
                $stock->setis_favorite_warehouse(($this->getRequest()->getPost('is_favorite_warehouse') == $stock->getId() ? 1 : 0));
            ...
            if ($associateWarehouseData) {
                if ($associateWarehouseData['warehouse_id']) {
                    ...
                    if ($isFavorite)
                        $newStockItem->setis_favorite_warehouse(1)->save();
                    if ($preferedStockLevel)
                        $newStockItem->setuse_config_notify_stock_qty(0)->setnotify_stock_qty($preferedStockLevel)->save();
                    if ($idealStockLevel)
                        $newStockItem->setuse_config_ideal_stock_level(0)->setideal_stock_level($idealStockLevel)->save();
                }
            }

            //save price
            if ($this->getRequest()->getPost('price_ttc')) {
        ...
        $this->_redirect('AdvancedStock/Products/Edit', array('product_id' => $productId, 'tab' => $currentTab));
    }

...
class MDN_AdvancedStock_WarehouseController extends Mage_Adminhtml_Controller_Action {
    ...
    public function DeleteAction() {
        ...
        if ($stockId <> 1) {
            $obj = mage::getModel('AdvancedStock/Warehouse')->load($stockId);
            $obj->delete();
            Mage::getSingleton('adminhtml/session')->addSuccess($this->__('Warehouse deleted'));
        } else {
            Mage::getSingleton('adminhtml/session')->addError($this->__('You can not delete default warehouse'));
        }
        $this->_redirect('*/*/Grid');
    }
    ...
    public function ExportStockAtDateAction() {
        ...
        $fileName = $this->__('Stock levels for %s at %s', $warehouse->getstock_name(), $date);

        $this->_prepareDownloadResponse($fileName, $content, 'text/plain');
    }

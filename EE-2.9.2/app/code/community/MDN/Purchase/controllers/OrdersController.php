...
class MDN_Purchase_OrdersController extends Mage_Adminhtml_Controller_Action {
     ...
     public function saveAction() {
         ...
         try {
             ...
             foreach ($products as $item) {
                 ...
                 // horsebrands dn: delete item when it is marked outofstock.
                 // log the removal in log file and po comments
                 if (isset($deliveryData[$item->getId()]['supplier_out_of_stock'])) {
                   $comment = $order->getData('po_comments') . ', ';
                   $comment .= $item->getSku() . ' nicht mehr lieferbar. ('.$deliveryDate.')';
                   $order->setpo_comments($comment);

                   Mage::log($order->getId().': '.$item->getSku().' nicht mehr lieferbar (Menge: '.$item->getpop_qty().')', null, 'po_supplier_outofstock.log');
    ...
    public function CommitScannerDeliveryAction() {
        ...
        foreach ($datas as $line) {
            ...
            $po->createDelivery($purchaseOrderItem, $data['scanned_qty'], date('Y-m-d'), 'Purchase order #' . $po->getpo_order_id(), $po->getTargetWarehouse()->getId(), false);

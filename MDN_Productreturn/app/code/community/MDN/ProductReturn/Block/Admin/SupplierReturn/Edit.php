...
class MDN_ProductReturn_Block_Admin_SupplierReturn_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
        ...
        if ($this->getRsr()->getId()) {
            $this->_addButton(
                ...
                array(
                    ...
                    'onclick' => "window.location.href='" . $this->getUrl('ProductReturn/SupplierReturn/Print', array('rsr_id' => $this->getRsr()->getId())) . "'"
                )
            );
        }
    }
    ...
    {
        return $this->getUrl('ProductReturn/SupplierReturn/Save');
    }

    public function getBackUrl()
    {
        return $this->getUrl('ProductReturn/SupplierReturn/index');
    }
}

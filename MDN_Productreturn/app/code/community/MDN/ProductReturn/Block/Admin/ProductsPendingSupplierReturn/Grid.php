...
class MDN_ProductReturn_Block_Admin_ProductsPendingSupplierReturn_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addExportType('ProductReturn/ProductsPendingSupplierReturn/exportCsv/', Mage::helper('customer')->__('CSV'));
    ...
    protected function _prepareMassaction()
    {
        ...
        $this->getMassactionBlock()->addItem('create_supplierReturn', array(
            ...
            'url'   => $this->getUrl('ProductReturn/ProductsPendingSupplierReturn/MassCreateSupplierReturn'),
        ));

        $this->getMassactionBlock()->addItem('destroy_product', array(
            'label' => Mage::helper('ProductReturn')->__('Destroy Product'),
            'url'   => $this->getUrl('ProductReturn/ProductsPendingSupplierReturn/MassDestroyProducts'),
        ));

        $this->getMassactionBlock()->addItem('assign_supplier', array(
            'label'      => Mage::helper('sales')->__('Assign a supplier'),
            'url'        => $this->getUrl('ProductReturn/ProductsPendingSupplierReturn/MassAssignSupplier'),
            'additional' => array(
                'supplier' => array(
                    'name'   => 'supplier',
                    'type'   => 'select',

...
class MDN_ProductReturn_Block_Productreturn_Edit_Tab_Actions extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    {
        return $this->getUrl('ProductReturn/Admin/ProductExchangeSelectionPopup', array('rma_id' => $this->getRma()->getId(),  'rp_id' => 'XXX'));
    }
    ...
    public function getProceedInformation($product)
    {
        ...
        $value .= ' - ' . mage::helper('ProductReturn')->__($product->getrp_destination());
    ...
    // horsebrands
    public function isOrderFullyReturned() {
      $helper = mage::helper('ProductReturn');

      $allinrma     = array();
      $itemsinorder = $this->getRma()->getSalesOrder()->getItemsCollection();

      foreach ($itemsinorder as $iteminorder):
          $allinrma[$iteminorder->getProductId()] = $iteminorder->getQtyShipped();
          $allinrma[$iteminorder->getProductId()] -= $helper->getIsalreadyInRma($iteminorder);
      endforeach;

 ...
 */
abstract class MDN_ProductReturn_Model_Pdf_Pdfhelper extends Mage_Sales_Model_Order_Pdf_Abstract
{
    ...
    public function widthForStringUsingFontSize($string, $font, $fontSize)
    {
        try {
            ...
            die("Erreur dans Mdn pdf helper m�thode widthForStringUsingFontSize avec string = " . $string);
        }
    }
     ...
     * @param string|\unknown_type $alignment
    ...
    public function drawFooter(&$page)
    {
        ...
        $this->DrawMultilineText($page, Mage::getStoreConfig('productreturn/pdf/footer_text'), 20, $this->_BLOC_FOOTER_HAUTEUR, 10, 0, 15);
    ...
    public function drawHeader(&$page, $title, $StoreId = null)
    {
        ...
        $this->DrawMultilineText($page, Mage::getStoreConfig('productreturn/pdf/header_text', $StoreId), 300, $this->y - 10, 10, 0, 15);
    ...
    public function WrapTextToWidth($page, $text, $width)
    {
        ...
        for ($i = 0; $i < count($t_words); $i++) {
            ...
            //si le mot contient un retour a la ligne, on remet la ligne courante � 0
            if (strpos($t_words[$i], "\n") === false) {
    ...
    public function AddAddressesBlock(&$page, $LeftAddress, $RightAddress, $TxtDate, $TxtInfo)
    {
        ...
        $page->drawLine($this->_PAGE_WIDTH / 2, $this->y, $this->_PAGE_WIDTH / 2, $this->y - 180);

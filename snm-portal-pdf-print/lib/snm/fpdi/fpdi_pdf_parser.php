...
class snmfpdi_pdf_parser extends pdf_parser {
     ...
     */
    function snmfpdi_pdf_parser($filename, &$fpdi) {
        $this->fpdi =& $fpdi;
        ...
        foreach ($filters AS $_filter) {
            switch ($_filter[1]) {
                ...
                case '/LZWDecode':
                    ...
                    $decoder = new FilterLZW_SNMFPDI($this->fpdi);
                    $stream = $decoder->decode($stream);
                    break;
                case '/ASCII85Decode':
                    include_once('filters/FilterASCII85_FPDI.php');
                    $decoder = new FilterASCII85_SNMFPDI($this->fpdi);
                    $stream = $decoder->decode($stream);
                    break;
                case null:
                    $stream = $stream;

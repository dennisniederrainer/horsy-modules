...
class SNMTCPDF {
	...
	protected $auto_stretching=0;//MAU
	/**
	 * Current page number.
	 * @protected
	 */
	 ...
	 */
	protected $cell_height_ratio = SNMK_CELL_HEIGHT_RATIO;
	/**
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false) {
		...
		$this->unicode = new SNMTCPDF_UNICODE_DATA();
	...
	public function Header() {
		if ($this->header_xobjid < 0) {
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
				$imgtype = $this->getImageFileType(SNMK_PATH_IMAGES.$headerdata['logo']);
				if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
					$this->ImageEps(SNMK_PATH_IMAGES.$headerdata['logo'], '', '', $headerdata['logo_width']);
				} elseif ($imgtype == 'svg') {
					$this->ImageSVG(SNMK_PATH_IMAGES.$headerdata['logo'], '', '', $headerdata['logo_width']);
				} else {
					$this->Image(SNMK_PATH_IMAGES.$headerdata['logo'], '', '', $headerdata['logo_width']);
				}
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					...
					if (defined('SNMK_THAI_TOPCHARS') AND (SNMK_THAI_TOPCHARS == true)) {
	...
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false) {
		...
		if ($resth > 0) {
			...
			while ($tmpresth > 0) {
				...
				$otmpresth = $tmpresth; //AUIT
				if ($this->num_columns > 1) {
					$tmpresth -= ($this->h - $this->y - $this->bMargin);
				} else {
					$tmpresth -= ($this->h - $this->tMargin - $this->bMargin);
				}
				if ( $otmpresth <= $tmpresth )//AUIT
				{
					//	Mage::log("resth : $resth :  $otmpresth h:{$this->h} - Y: {$this->y} bMargin:{$this->bMargin}<= $tmpresth => stop");
					break;
				}
				--$skip;
			}
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if ($file{0} === '@') {
			...
			$file = SNMK_PATH_CACHE.'img_'.md5($imgdata);
			$fp = fopen($file, 'w');
			fwrite($fp, $imgdata);
			fclose($fp);
			unset($imgdata);
...
//MAU
					...
					curl_setopt($cs, CURLOPT_MAXREDIRS, 10);
		...
		if ($imsize === FALSE) {
			if (substr($file, 0, -34) == SNMK_PATH_CACHE.'msk') { // mask file
				// get measures from specified data
				$pw = $this->getHTMLUnitToUnits($w, 0, $this->pdfunit, true) * $this->imgscale * $this->k;
				$ph = $this->getHTMLUnitToUnits($h, 0, $this->pdfunit, true) * $this->imgscale * $this->k;
				$imsize = array($pw, $ph);
			} else {
				Mage::log('[Image] Unable to get image: '.$file);
				return false;
		...
		if (in_array($file, $this->imagekeys)) {
			...
			if (substr($file, 0, -34) != SNMK_PATH_CACHE.'msk') {
		...
		} elseif (substr($file, 0, -34) != SNMK_PATH_CACHE.'msk') {
			// check for cached images with alpha channel
			$filehash = md5($file);
			$tempfile_plain = SNMK_PATH_CACHE.'mskp_'.$filehash;
			$tempfile_alpha = SNMK_PATH_CACHE.'mska_'.$filehash;
		...
		if ($newimage) {
			...
			if (!$info) {
				if (function_exists($gdfunction)) {
					...
					$tempname = tempnam(SNMK_PATH_CACHE, 'jpg_');
	 ...
	 * This function requires GD library and write access to the directory defined on SNMK_PATH_CACHE constant.
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			$tempdoc = tempnam(SNMK_PATH_CACHE, 'tmppdf_');
			$f = fopen($tempdoc, 'wb');
			if (!$f) {
			...
			$tempsign = tempnam(SNMK_PATH_CACHE, 'tmpsig_');
	...
	protected function _getfontpath() {
		if (!defined('SNMK_PATH_FONTS') AND is_dir(dirname(__FILE__).'/fonts')) {
			define('SNMK_PATH_FONTS', dirname(__FILE__).'/fonts/');
		}
		return defined('SNMK_PATH_FONTS') ? SNMK_PATH_FONTS : '';
	}
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			...
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				...
				$tempkeyfile = tempnam(SNMK_PATH_CACHE, 'tmpkey_');
				...
				fclose($f);
				$tempencfile = tempnam(SNMK_PATH_CACHE, 'tmpenc_');
	...
	public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres='', $style='', $align='') {
		...
		$barcodeobj = new SNMTCPDFBarcode($code, $type);
	...
	public function write2DBarcode($code, $type, $x='', $y='', $w='', $h='', $style='', $align='', $distort=false) {
		...
		$gvars = $this->getGraphicVars();
		// create new barcode object
		$barcodeobj = new SNMTCPDF2DBarcode($code, $type);
		$arrcode = $barcodeobj->getBarcodeArray();
	...
	protected function getHtmlDomArray($html) {
		...
		$dom[$key]['line-height-after'] = '10';//MAU
		$dom[$key]['auto-stretching'] = '0';//MAU
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element{0} == '/') {
					...
					if (isset($dom[$key]['attribute']['style']) AND !empty($dom[$key]['attribute']['style'])) {
						...
						//MAU
						if (isset($dom[$key]['style']['auto-stretching'])) {
							$dom[$key]['auto-stretching'] = $dom[$key]['style']['auto-stretching'];
						}
						if (isset($dom[$key]['style']['line-height-after'])) {
							$lineheight = trim($dom[$key]['style']['line-height-after']);
							switch ($lineheight) {
								// A normal line height. This is default
								case 'normal': {
									$dom[$key]['line-height-after'] = $dom[0]['line-height-after'];
									break;
								}
								default: {
									if (is_numeric($lineheight)) {
									...
									$dom[$key]['line-height-after'] = $this->getHTMLUnitToUnits($lineheight, 1, '%', true);
								}
							}
						}
						if (isset($dom[$key]['style']['line-height-before'])) {
							$lineheight = trim($dom[$key]['style']['line-height-before']);
							switch ($lineheight) {
								// A normal line height. This is default
								case 'normal': {
									$dom[$key]['line-height-before'] = $dom[0]['line-height-before'];
									break;
								}
								default: {
									if (is_numeric($lineheight)) {
									...
									$dom[$key]['line-height-before'] = $this->getHTMLUnitToUnits($lineheight, 1, '%', true);
								}
							}
						}
						//MAU END
						...
						if (isset($dom[$key]['style']['page-break-before'])) {
							if ($dom[$key]['style']['page-break-before'] == 'always') {
								...
								$dom[$key]['attribute']['pagebreak'] = $this->getHTMLUnitToUnits($dom[$key]['style']['page-break-before'], 1, 'px', false);
							}//MAU END
						...
						if (isset($dom[$key]['style']['page-break-after'])) {
							if ($dom[$key]['style']['page-break-after'] == 'always') {
								...
								$dom[$key]['attribute']['pagebreakafter'] = $this->getHTMLUnitToUnits($dom[$key]['style']['page-break-after'], 1, 'px', false);
							}//MAU END
						}
					}
					...
					if (($dom[$key]['value'] == 'small') OR ($dom[$key]['value'] == 'sup') OR ($dom[$key]['value'] == 'sub')) {
						if (!isset($dom[$key]['attribute']['size']) AND !isset($dom[$key]['style']['font-size'])) {
							$dom[$key]['fontsize'] = $dom[$key]['fontsize'] * SNMK_SMALL_RATIO;
						}
					}
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		while ($key < $maxel) {
			...
			//MAU START
			if ( is_numeric($dom[$key]['attribute']['pagebreak']) )
			{
				$this->checkPageBreak($dom[$key]['attribute']['pagebreak']);
			}
			//MAU END
...
//				if ( isset($dom[$key]['auto-stretching']) )
	//				$this->auto_stretching = $dom[$key]['auto-stretching'];
			...
			if ($dom[$key]['tag']) {
				...
				if (!isset($dom[$key]['trimmed_space']) OR !$dom[$key]['trimmed_space']) {
					...
					if (!empty($this->HREF) AND (isset($this->HREF['url']))) {
						...
						// chk for reversed text direction
						...
						//MAU AUIT START
						$saveStretching=$this->font_stretching;
						if ( $strlinelen > $cwa   )
						{
							
							if ( (isset($dom[$dom[$key]['parent']]['style']['auto_stretching'])
								&&  $dom[$dom[$key]['parent']]['style']['auto_stretching'] == 1)
								||
								(isset($dom[$dom[$key]['parent']]['style']['auto-stretching'])
								&&  $dom[$dom[$key]['parent']]['style']['auto-stretching'] == 1)
								|| $this->auto_stretching
									
								 )
							{
									$streching= $this->font_stretching;
									while ( $streching > 0 )
									{
										$streching -= 1;
										$this->font_stretching=$streching;
										$strlinelen = $this->GetStringWidth($dom[$key]['value']);
										if ( $strlinelen <= $cwa ) break;
									}
							}
							else  if ( $loop > 0)
							{
						...
						//MAU AUIT START
						$this->font_stretching=$saveStretching;
						//MAU AUIT END
				...
				if (strlen($strrest) > 0) {
					...
					if ($strrest == $dom[$key]['value']) {
						...
						++$loop;
//						Mage::log("---- Rest ist gleich :".$strrest." : ".$this->y);
				//		$this->y -= $this->lasth;
					} else {
						$loop = 0;
					}
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		if ($tag['block']) {
			...
			// MAU
			if ( isset($tag['line-height-before']) && !is_null($tag['line-height-before']))
			{
				$hbz = $this->FontSize * $tag['line-height-before'];
			}
		}
		// Opening tag
		switch($tag['value']) {
			...
			/***
			case 'barcode':{ //MAU
				if (isset($tag['attribute']['code']) && isset($tag['attribute']['type']) ) {
					$iw = '';
					if (isset($tag['width'])) {
					...
					$it=true;
					if (isset($tag['attribute']['text'])) {
						if ( $tag['attribute']['text'] != 'true' && $tag['attribute']['text'] != '1' )
							$it=false;
					}
					$style = array(
						'position' => 'L',				'align' => 'L',				'stretch' => false,				'fitwidth' => false,
						'cellfitalign' => '',				'border' => false,				'padding' => 0,
						'fgcolor' => array(0,0,0),
					...
					$style = array(
						...
						'border' => false,
						'padding' => 0,
						'fgcolor' => array(0,0,0), 'bgcolor' => false,				'text' => $it
					);

					$code = $tag['attribute']['code'];
					$type = $tag['attribute']['type'];

					//$this->write1DBarcode($code, $type,'','',$iw,$ih,'',$style,'');
			...
			case 'barcode2d':{ //MAU
				if (isset($tag['attribute']['code']) && isset($tag['attribute']['type']) ) {
					...
					$style = array(
						...
						'border' => false,
						'padding' => 0,
						'fgcolor' => array(0,0,0), 'bgcolor' => false,				'text' => true
					);
					...
					$this->write2DBarcode($code, $type,'','',$iw,$ih,'',$style,'');
			...
			*/
			case 'img': {
				if (isset($tag['attribute']['mode']) ) {
					...
					$ib=false;
					if (isset($tag['attribute']['border'])) {
						if ( $tag['attribute']['border'] != 'false' && $tag['attribute']['border'] != '0' )
							$ib=true;
					}
					$ip=0;
					if (isset($tag['attribute']['padding'])) {
						if ( $tag['attribute']['padding'] > 0 )
							$ip=(float)$tag['attribute']['padding'];
					}
					$ipo='S';
					if (isset($tag['attribute']['position'])) {
						$ipo=$tag['attribute']['position'];
					}

					if (isset($tag['attribute']['code']) && isset($tag['attribute']['type']) ) {
						if ( $tag['attribute']['mode'] =='barcode' )
						{
							$this->write1DBarcode(	stripslashes($tag['attribute']['code']),
											$tag['attribute']['type'], '', '',
											$iw,$ih, '',
											array(	'position'=>$ipo,
													//'align' => 'L',
						...
						if ( $tag['attribute']['mode'] =='barcode2d' )
						{
							$this->write2DBarcode(	stripslashes($tag['attribute']['code']),
											$tag['attribute']['type'], '', '',
											$iw,$ih, '',
											array(	'position'=>$ipo,
													//'align' => 'L',
				...
				}else if (isset($tag['attribute']['src'])) {
					...
					if (!isset($testscrtype['query']) OR empty($testscrtype['query'])) {
						...
						$tag['attribute']['src'] = str_replace(SNMK_PATH_URL, SNMK_PATH_MAIN, $tag['attribute']['src']);
					}
					if (!isset($tag['width'])) {
						$tag['width'] = 0;
					}
			...
			case 'form': {
				if (isset($tag['attribute']['action'])) {
					...
					$this->form_action = SNMK_PATH_URL.$_SERVER['SCRIPT_NAME'];
				}
			...
			case 'tcpdf': {
				if (defined('SNMK_TCPDF_CALLS_IN_HTML') AND (SNMK_TCPDF_CALLS_IN_HTML === true)) {
	...
	protected function closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney=0) {
		...
		if ($tag['block']) {
			...
			// MAU
			if ( isset($parent['line-height-after']) && !is_null($parent['line-height-after']))
			{
				$hbz = $this->FontSize * $parent['line-height-after'];
			}
		}
		// Closing tag
		switch($tag['value']) {
	...
	protected function getObjFilename($name) {
		return tempnam(SNMK_PATH_CACHE, $name.'_');
	}
		...
		switch($name) {
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (!isset($testscrtype['query']) OR empty($testscrtype['query'])) {
						// convert URL to server path
						$img = str_replace(SNMK_PATH_URL, SNMK_PATH_MAIN, $img);
					}
